...
if path.exists(config_file): 
    ...
    snscrape = podrennes_config.get('snscrape')
    ftp_upload = podrennes_config.get('ftp_upload') == '1'
    ...
    if hashtags:
        if ftp_is_connected(ftp_host, ftp_login, ftp_pass, ftp_dir):
            ...
            for hashtag in hashtags:
                print(f"Recherche de {hashtag} sur FTP {ftp_host} / Keep Local : {keep_local} / Reset : {reset} / Upload : {ftp_upload} / Command : {snscrape}", file = log_file)
                ...
                scrapper.run(hashtag, snscrape, ftp_upload)
        else:
            log_file = open(log_file_name, 'a')
